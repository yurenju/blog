...
**實作備註**
[方向調整] 原本 `getAllPostMetadata()` 中無 `.md` 檔案時會拋出錯誤，改為靜默跳過（`return`），因為兩層結構中文章目錄內的子目錄（如 `assets/`）不含 `.md` 檔案，不應報錯。

[後續依賴] `getGroupFromFilePath()` 從檔案路徑中推算 group 名稱，依賴 `public/posts/{group}/{postDir}/{file}.md` 的路徑結構。遷移腳本必須確保此結構。
